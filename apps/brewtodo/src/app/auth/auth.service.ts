...
export class AuthService {
    ...
    public isAdmin(): boolean {
        if (this._accessToken) {
            ...
            if (
                decodedToken['http://localhost:4200/api/roles'].indexOf(
                    'admin'
                ) > -1
            ) {
                return true
    ...
    public upsertUser(username: string): void {
        ...
        const userUpdate = this.http
            .post<boolean>('/api/user', body, this._authHeader())
            .subscribe((res: any) => {
                this.currentUserId = res.id
                this.sendNotification()
                return userUpdate.unsubscribe()
            })
    }

    sendNotificationOfAuth(): Observable<any> {
